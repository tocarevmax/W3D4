...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where('castings.ord != 1 AND name = \'Harrison Ford\'')
end
...
def biggest_cast
  ...
  Movie.select(:id,:title).joins(:actors).group(:id).order('COUNT(actors.id) DESC').limit(3)
